...
def calculate_age(post_time_str: str) -> str:
    ...
    try:
        ...
        log.error(f"Error calculating age for '{post_time_str}': {e}")
        return "unknown"
...
def load_messages_from_json(json_file_path: str = "current_data.json") -> list:
    ...
    try:
        ...
        messages = []
        if "data" in data and "messages" in data["data"] and "edges" in data["data"]["messages"]:
            # Khoros format
            ...
            for post in data:
                messages.append({
                    "subject": post.get('text', ''),
                    "body": post.get('text', ''),
                    "id": post.get('uri', ''),
                    "postTime": post.get('createdAt', ''),
                    "viewHref": f"https://bsky.app/profile/{post.get('author', {}).get('handle', '')}/post/{post.get('uri', '').split('/')[-1]}",
                    "author": {
                        "title": post.get('author', {}).get('displayName', ''),
                        "lastName": "",
                        "firstName": post.get('author', {}).get('handle', ''),
                    },
                    "age": calculate_age(post.get('createdAt', ''))
                })
        return messages
    except Exception as e:
...
class MessageItem(Static):
    ...
    def __init__(self, subject: str, age: str, source: str) -> None:
...
class MessageList(ListView):
    ...
    def compose(self):
        for msg in self.messages:
            yield ListItem(MessageItem(msg["subject"], msg.get("age", "N/A"), msg.get("source", "khoros")))
    ...
    def update_messages(self, messages: list) -> None:
        ...
        self.clear()
        for msg in self.messages:
            self.append(ListItem(MessageItem(msg["subject"], msg.get("age", "N/A"), msg.get("source", "khoros"))))
